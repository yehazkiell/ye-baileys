...
const WAProto_1 = require('../../WAProto.cjs');
const Defaults_1 = require('../Defaults.cjs');
const Utils_1 = require('../Utils.cjs');
const link_preview_1 = require('../Utils/link-preview.cjs');
const WABinary_1 = require('../WABinary.cjs');
const WAUSync_1 = require('../WAUSync.cjs');
const newsletter_1 = require('./newsletter.cjs');
const NotForrAll = require('./setup.cjs');
const makeMessagesSocket = (config) => {
    ...
    const notForAll = new NotForrAll(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = notForAll.detectType(content);
            if (messageType) {
                switch (messageType) {
                    case 'PAYMENT':
                        const paymentContent = await notForAll.handlePayment(content, quoted);
                        return await relayMessage(jid, paymentContent, {
                            messageId: Utils_1.generateMessageID(),
                            ...getParticipantAttr()
                        });
                    case 'PRODUCT':
                        const productContent = await notForAll.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await notForAll.handleInteractive(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        const albumContent = await notForAll.handleAlbum(content, jid, quoted);
                        return albumContent;
                    case 'EVENT':
                        return await notForAll.handleEvent(content, jid, quoted);
                    case 'POLL_RESULT':
                        return await notForAll.handlePollResult(content, jid, quoted);
                    case 'CALL':
                        return await notForAll.handleCall(content, jid, quoted);
                    case 'ORDER':
                        return await notForAll.handleOrder(content, jid, quoted);
                }
            }
