const WAProto = require('../../WAProto.cjs').proto;
...
class NotForrAll {
    ...
    detectType(content) {
        ...
        if (content.callMessage) return 'CALL';
    ...
    async handlePayment(content, quoted) {
        ...
        async handleCall(content, jid, quoted) {
            const callData = content.callMessage;
            const msg = await this.utils.generateWAMessageFromContent(jid, {
                scheduledCallCreationMessage: {
                    scheduledTimestampMs: callData.time || Date.now(),
                    callType: callData.type || 1,
        ...
        async handleOrder(content, jid, quoted) {
            ...
            const msg = await this.utils.generateWAMessageFromContent(jid, {
                orderMessage: {
                    ...
                    surface: orderData.surface || 1,
                    orderTitle: orderData.title,
                    message: orderData.text,
                    sellerJid: orderData.seller,
                    token: orderData.token,
                    totalAmount1000: orderData.amount,
